...
vardict.dir <- '/ifs/work/bergerm1/zhengy1/RET_all/snv_pipeline/run_091619/ret_091619-1.2.4/'
...
# data from DMP -----------------------------------------------------------
DMP.maf <- fread('/ifs/work/bergerm1/zhengy1/dmp/mskimpact/data_mutations_extended.txt') %>% filter(Mutation_Status == 'SOMATIC') %>% data.table()
dim(DMP.maf)
...
total.maf <- do.call(rbind,lapply(list.files(vardict.dir,pattern = 'combined-variants.vep_keptrmv_taggedHotspots.maf$',recursive = F,full.names = T),function(x){
  fread(x) %>% filter(as.numeric(t_alt_count) > 0) %>% data.table()
})) %>% mutate(Tumor_Sample_Barcode = gsub('_S.._001.*|_cl.*','',Tumor_Sample_Barcode)) %>% data.table()
# unique(total.maf$Tumor_Sample_Barcode)

# Pooled normal samples ---------------------------------------------------
pooled.bams <- list.files('/ifs/work/bergerm1/zhengy1/RET_all/all_bams/curated_duplex/',pattern = 'DONOR.*.bam',full.names = T)


# helper ret maf ----------------------------------------------------------
ret.helper.maf = fread('/ifs/work/bergerm1/zhengy1/RET_all/Original_files/helper_maf/ret_helper_maf2maf.maf') %>% mutate(
  Tumor_Sample_Barcode = case_when(
    HGVSp_Short == "p.V804M" ~ 'C-H77MCH',HGVSp_Short == "p.K666N" ~ 'C-HAT9MW',HGVSp_Short == "p.M918T" ~ 'C-2UW6JP',
    HGVSp_Short == "p.C634R" ~ 'C-YW82CY',HGVSp_Short == "p.C634Y" ~ 'C-9DRD2D'
  )
) %>% data.table()
...
x =  "C-0EU9LX"
x = 'C-93W8WP'
# master.ref = master.ref[`CMO patient ID` == 'C-93W8WP']
all.fillout.id <- lapply(unique(master.ref$`CMO patient ID`),function(x){
  ...
  if(x %in% ret.helper.maf$Tumor_Sample_Barcode){
    print(paste0('On patient ',x))
    print(paste0('Adding event ',ret.helper.maf[Tumor_Sample_Barcode == x,colnames(all.calls), with = FALSE]$HGVSp_Short))
    all.calls = rbind(all.calls,ret.helper.maf[Tumor_Sample_Barcode == x,colnames(all.calls), with = FALSE])
  }
  ...
  system(paste0(
    ...
    ' /home/johnsoni/virtualenvs/pipeline_1.2.4/bin/python /ifs/work/bergerm1/zhengy1/ACCESS-Pipeline-DEV/cwl_tools/hotspots/tag_hotspots.py ',
...
pooled.bams <- list.files('/ifs/work/bergerm1/zhengy1/RET_all/all_bams/curated_duplex',pattern = '.bam',full.names = T)
